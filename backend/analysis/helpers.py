...
def get_task_dates(priority, due, dev, only_positive=None, is_bind=True):
    ...
    while True:
        ...
        if 20 > start_time.hour > 6:
            ...
            if priority>4:
                ...
                priority_coeff = 0.7
            if is_bind:
                end_time = start_time + timedelta(minutes=generate_random_val(due, dev, only_positive)*priority_coeff)
            else:
                end_time = None
...
users_pressets = {
    "Василёк": {"Фикс чего-то там!": (1000, True), "Оформить заявку!": (1000, True), "проверка склада!": (1000, True), "Перепиши весь десигн!": (1000, True), "Сделай гугл!": (1000, True)},
...
#            continue
        if assigner_task_preset[0] < 100:
            is_bind = True
        start, due, end = get_task_dates(task_priority, task_due_minutes, *assigner_task_preset, is_bind)
        task_list.append(TaskRawData(process_name, task_name, owner, assigner, start, end, due, task_priority, task_var_count, is_bind))
